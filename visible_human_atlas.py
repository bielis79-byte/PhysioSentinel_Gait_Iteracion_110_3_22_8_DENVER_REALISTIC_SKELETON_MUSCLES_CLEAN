...
# ---------------------------------------------------------------------------
# V110.3.22.8 · DENVER REAL BONE ATLAS
...
def build_denver_bone_master(man,max_faces_each=70):
    ...
    root=Path(man["root"])
    cache=root/f"denver_real_bones_master_v110_3_22_8_{max_faces_each}.npz"
    meta=root/f"denver_real_bones_master_v110_3_22_8_{max_faces_each}.json"
    ...
    for p in parts:
        ...
        if len(V)==0 or len(F)==0:
            continue
        step=max(1,int(np.ceil(len(F)/float(max_faces_each))))
        Fd=np.asarray(F[::step,:3],np.int32)
        used=np.unique(Fd.reshape(-1))
        rem=np.full(len(V),-1,np.int32)
        rem[used]=np.arange(len(used),dtype=np.int32)
        Vd=V[used]
        Vs.append(Vd.astype(np.float32))
        Fs.append(rem[Fd]+off)
